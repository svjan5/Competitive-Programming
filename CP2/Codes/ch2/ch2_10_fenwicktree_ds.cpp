...
class FenwickTree {
        ...
        public:
                FenwickTree() {}
                FenwickTree(int n) { ft.assign(n+1, 0); }               /*initialization: n + 1 zeroes, ignore index 0*/

                int rsq(int b) {                                        /*query RSQ(1, b)*/
                        int sum = 0; for (; b; b -= LSOne(b)) sum += ft[b];
                        return sum; 
                }

                int rsq(int a, int b) {                                 /*query RSQ(a, b)*/
                        return rsq(b) - (a == 1 ? 0 : rsq(a - 1)); 
                }

                void adjust(int k, int v) {                             /*update the kth element by v*/
